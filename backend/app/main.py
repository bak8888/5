...
TEAM_STATS = {
    # attack, defense, form, recent_5_points, gf10, ga10, home_score, away_score, h2h_score, style
    "서울 졸라FC": (.78, .64, .72, 10, 1.8, 1.1, .7, .48, .58, "high_pressing"),
    "부산 스틱맨즈": (.62, .58, .55, 6, 1.2, 1.5, .52, .44, .42, "counter_attack"),
    "레드 라인즈": (.83, .69, .8, 12, 2.0, .9, .76, .62, .61, "possession"),
    "블루 서클스": (.58, .61, .52, 5, 1.0, 1.3, .5, .4, .39, "low_block"),
    "마드리드 막대": (.7, .66, .64, 8, 1.5, 1.0, .67, .55, .52, "possession"),
    "카탈루냐 드리블": (.74, .6, .69, 9, 1.7, 1.2, .63, .57, .48, "high_pressing"),
}
...
def seed(db: Session):
    if db.query(Match).first():
        return
    teams = {}
    for name, (attack, defense, form, recent_points, gf10, ga10, home_score, away_score, h2h_score, style) in TEAM_STATS.items():
        ...
        db.add(TeamStats(
            ...
            recent_10_avg_goals_for=gf10,
            recent_10_avg_goals_against=ga10,
            home_record_score=home_score,
            away_record_score=away_score,
            h2h_score=h2h_score,
            team_style=style,
        ))
        teams[name] = team
