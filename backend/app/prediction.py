...
FORBIDDEN_EXPLANATION_TERMS = ("확실하다", "무조건", "보장")
...
def _safe_odd(value: float) -> float:
    return max(value, 1.01)

def _rounded_probability_triplet(home_prob: float, draw_prob: float, away_prob: float) -> tuple[float, float, float]:
    rounded_home = round(home_prob, 4)
    rounded_draw = round(draw_prob, 4)
    rounded_away = round(1 - rounded_home - rounded_draw, 4)
    if rounded_away < 0:
        rounded_away = 0.0
        rounded_home, rounded_draw, rounded_away = [round(value, 4) for value in _normalize([rounded_home, rounded_draw, rounded_away])]
    return rounded_home, rounded_draw, rounded_away

def _sanitize_explanation(explanation: str) -> str:
    sanitized = explanation
    for term in FORBIDDEN_EXPLANATION_TERMS:
        sanitized = sanitized.replace(term, "단정하지 않습니다")
    return sanitized

def _implied_probabilities(odds: OddsInput) -> tuple[float, float, float]:
    home, draw, away = _normalize([1 / _safe_odd(odds.home), 1 / _safe_odd(odds.draw), 1 / _safe_odd(odds.away)])
...
def run_prediction_engine(engine_input: PredictionEngineInput) -> PredictionEngineOutput:
    ...
    h2h_home = _clamp((home.h2h_score + (1 - away.h2h_score) + engine_input.match_context.neutral_h2h_score) / 3)
    h2h_away = 1 - h2h_home
    ...
    rounded_home, rounded_draw, rounded_away = _rounded_probability_triplet(home_prob, draw_prob, away_prob)
