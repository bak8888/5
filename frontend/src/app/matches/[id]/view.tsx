      ...
      {prediction && (
        <section className="mt-8 rounded-2xl bg-slate-900 p-5">
          ...
          <div className="mt-4 grid gap-2 text-sm text-slate-300 sm:grid-cols-3">
            <span>예상 득점: 홈 {prediction.expectedHomeGoals?.toFixed(2) ?? '-'} · 원정 {prediction.expectedAwayGoals?.toFixed(2) ?? '-'}</span>
            <span>신뢰도: {prediction.confidence !== undefined ? `${(prediction.confidence * 100).toFixed(1)}%` : '-'}</span>
            <span>스타일: {prediction.home_style} vs {prediction.away_style}</span>
          </div>
        </section>
      )}
